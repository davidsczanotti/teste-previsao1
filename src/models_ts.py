...
def nhits_cfg():
    return NHITS(h=5, input_size=60, max_steps=200, n_blocks=3)

def nbeats_cfg():
    return NBEATS(h=5, input_size=60, max_steps=200, stack_types=['trend','seasonality'])

def patchtst_cfg():
    return PatchTST(h=5, input_size=96, max_steps=200)
...
def make_tft_dataset(df, group='ticker', time='date', target='ret_fwd'):
    ...
    training = TimeSeriesDataSet(
        df,
        time_idx=time, target=target, group_ids=[group],
        max_encoder_length=60, max_prediction_length=5,
        time_varying_known_reals=[], time_varying_unknown_reals=[target, "rsi_14", "MACD_12_26_9"],
